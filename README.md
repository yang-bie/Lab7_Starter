...
1. I would fit my automated tests within a GitHub Action that runs whenever code is pushed.
   This is the best place because the test will be run automatically every time new code is added to the repo. It helps catch bugs early and makes sure the project still works after changes are made. Compared with only running tests manually, GitHub Action is more reliable because developers might forget to run tests before pushing. This also helps prevent broken code from being added to the project.
2. No. 
   End-to-end tests are mainly used to test the whole user flow. If I only want to check whether one function returns the correct output, I should use a unit test instead.
